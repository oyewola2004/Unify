...
namespace UNIFY.Implementation.Services
{
    public class CommunityService : ICommunityService
    {
        ...
        public async Task<CommunitiesResponseModel> GetCommunitiesByName(string name)
        {
           var communities = await _communityRepository.GetCommunitiessByName(name);
            if (communities == null)
            {
                return new CommunitiesResponseModel
                {
                    Message = $"No Community name with {name} available",
                    Status = false
                };
            }
            var communityDtos = communities.Select(communities => new CommunityDto
            ...
            {
                Data = communityDtos,
                Message = $"List of all community with {name} retrieved ",
                Status = true
            };
        }
    }
