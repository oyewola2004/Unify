...
namespace UNIFY.Controllers
{
    ...
    public class CommunityController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetApartmentsByLGA(string LGA)
        {
         ...
         [HttpGet("GetCommunitiesByName")]
        public async Task<IActionResult> GetApartmentsByName(string name)
        {
            var communities = await _communityService.GetCommunitiesByName(name);
            if (communities.Status)
            {
                return Ok(communities);
            }
